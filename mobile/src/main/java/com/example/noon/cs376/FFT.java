...
/**
 * Created by estra on 11/28/2017.
 */
...
public class FFT {
    ...
    public FFT(int n) {
        ...
        if(n != (1<<m))
            throw new RuntimeException("FFT length must be power of 2. n=" + n + ", m=" + m);
...
//     }
    ...
    public static double[] computeMagnitude(double[] x, double[] y)
    {
        double[] mag = new double[x.length];
        for (int i = 0; i < x.length; i++) {
            mag[i] = Math.pow(x[i], 2) + Math.pow(y[i], 2);
        }
        return mag;
    }
